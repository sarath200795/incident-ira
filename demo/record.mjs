...
const outFile = path.join(outDir, 'incident-ira.mp4')
const silentFile = path.join(outDir, 'incident-ira.silent.mp4')
...
const t0 = Date.now()
const rec = await startScreencast(page, path.join('/tmp', 'frames-incident-ira'))
initAudio(path.join('/tmp', 'audio-incident-ira'), t0)
...
if (res) {
  ...
  const wav = path.join('/tmp', 'audio-incident-ira', 'narration.wav')
